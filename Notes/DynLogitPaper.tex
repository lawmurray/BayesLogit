...
\author{ %
  ...
  Liang Sun %
  \footnote{ %
    Jesse Windle is a Post Doctoral Associate in the Department of Statistical
    Science, Duke University, Box 90251, Durham NC 27708-0251 (E-mail:
    jesse.windle@stat.duke.edu).
    % 
...
\abstract{Dynamic linear models with Gaussian observations and Gaussian states
  ...
  are small.  Supplementary files are provided for replicating the benchmarks.
...
\npoint Non real-valued data arises in a variety of settings.  For instance,
integral data may be found in ecology or epidemiology when modeling populations
of species or outbreaks of infections and in neuroscience when modeling brain
activity in terms of discrete neural impulses.  Categorical data is similarly
ubiquitous, arising, for example, in medicine to describe a successful treatment
and in economics to denote the default on a loan.
%%%%
\npoint Such data may be temporally correlated, in which case setting up an
autoregressive-like structure is helpful, though at the cost of making posterior
inference more difficult.
%%%%
\npoint We address this hardship, proposing an elegant and efficient data
augmentation technique for dynamic models of counts or categories that have
binomial likelihoods.
%%%%
\npoint The origin of these models can be traced to their static ancestors,
which, in turn, are successors to the linear model.  Currently, we provide a
