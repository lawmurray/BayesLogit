...
% \npoint Let us continue to assume that $y_t$ is the response, generated by some
% member of the exponential family according to $f(y_t | \psi_t)$ where $\psi_t =
% \vx_t' \bbeta_t$ for $t=1, \ldots, T$, and that the hidden states
...
One may assess the \Polya-Gamma approach along two axes: ease of use and
...
The \Polya-Gamma approach, meanwhile, is simple in both concept and practice.
...
% \npoint The merits of our approach come from its efficiency in addition to its
% simplicity and flexibility.  To provide an extensive suite of benchmarks against
% against competing methods (\cite{fussl-etal-2013, fruhwirth-schnatter-etal-2009,
%   migon-etal-2013}) and show that the \Polya-Gamma approach is the best choice
% for dynamic logistic regression models and is the best choice for dynamic
% negative binomial regression when the count size is small.
...
The \Polya-Gamma data augmentation approach to dynamic models of counts or
categories is elegant, efficient, and leads to familiar complete conditionals in
the quantity of interest, making it easy to implement and customize.  Thus, it
is an excellent choice for almost any modeling scenario.  For those concerned
most with computational efficiency, there is one caveat to this conclusion: the
approach of \cite{fruhwirth-schnatter-etal-2009} can lead to faster raw
computations, at the cost of the implementational challenges already described.
