...
\abstract{ Dynamic linear models with Gaussian observations and Gaussian states
  ...
  epidemiology, and neuroscience.  In this paper, we compare the performance of
  the \Polya-Gamma data augmentation technique of \cite{polson-etal-2012} with
  the discrete mixture of normals approach based of
  ...
  \smallskip
  \textbf{Keywords:} {binomial, dynamic generalized linear models, negative binomial}
}
...
\beta$ in a non-linear way \citep{mccullagh-nelder-1989}.  Extensions to the
dynamic case are straightforward via a dynamic regression coefficient: $\psi_t =
x_t' \beta_t$, where one must specify the dynamics of $\{\beta_t\}_{t=1}^T$ to
...
$\beta$ or $\{\beta_t\}_{t=1}^T$.  Thus, when generating posterior samples one
