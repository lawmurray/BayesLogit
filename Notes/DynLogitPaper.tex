...
There are a variety of settings in which one wants to model non real-valued
...
when modeling brain activity in terms of the discrete neural impulses.
Categorical data is similarly ubiquitious, arising, for example, in medicine to
describe a successful treatment and in economics to denote the default on a
loan.  Further, such data may be temporily correlated, in which case setting up
some sort of autoregressive-like structure is helpful, though at the cost of
making posterior inference more difficult.  This paper presents a data
augmentation technique that produces posterior samples for state-space models
with binomial or negative binomial observations and linear, Gaussian evolution
equations, which are useful in the aforementioned circumstances.  We begin with
a brief overview of the setup, starting with the static analog, building the
dynamic models, and then describing the challenges faced when generating
posterior samples.

When there is a linear relationship between the expectation of the response and
the covariates and the variance of the response is constant, that is when one is
doing linear regression, the Gauss-Markov theorem ensures that ordinary least
squares produces the best unbiased minimum variance estimator, a frequentist way
of saying that the model has a ``nice'' solution.  Assuming that the respnose is
normally distributed leads to a closed-form posterior distribution that is easy
to sample from, which is the Bayesian equivalent of ``niceness.''  When modeling
counts or categories, ordinary least squares no longer has this property and
convenient posterior distributions are not gaurenteed.
...
distributions, ``tractable'' means that one may easily calculate the posterior
mode and associated error estimates; however, most likelihoods lead to posterior
...
coefficients, $\{\beta_t\}_{t=1}^T$, so that at time $t$ is $\psi_t = x_t'
\beta_t$ while response is still distributed as $f(y_t | \psi_t)$.  One must
specify a prior for $\{\beta_t\}_{t=1}^T$ to complete the model.  The prior
controls how quickly $\{\beta_t\}_{t=1}^T$ may change.  It is common to let
...
states.  Moving from the static to the dynamic model is a small step
conceptually but a big step practically.  In particular, it becomes much more
