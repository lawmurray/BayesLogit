...
successful treatment and in economics to denote the default on a loan.  Such
data may be temporally correlated as well, in which case setting up an
autoregressive-like structure is helpful, though at the cost of making posterior
inference more difficult.  This paper presents a data augmentation technique
that produces posterior samples for
%state-space models with binomial or negative binomial observations and linear,
%Gaussian evolution equations,
models that are useful in the foregoing scenarios.  We begin by setting up the
problem, starting with the static analog, moving to the dynamic model, and then
...
%, focusing on binomial likelihoods.  
...
posterior simulation for dynamic models; Section 3 shows how one may use the
...
\npoint \cite{carter-kohn-1994} and \cite{fruhwirth-schnatter-1994} showed that
when $y_t \sim N(\psi_t | V_t)$ there is an algorithm, the FFBS, that generates
posterior samples of $\{\beta_{t}\}_{t=1}^T$ in $\mcO(T)$ time as $T$ varies.
When the response comes from some other distribution, like the binomial or
negative binomial distributions, the FFBS breaks down.
...
\npoint Such attempts date back to at least \cite{west-etal-1985}, who employed
a conjugate updating, backward sampling (CUBS) strategy to generate approximate
posterior draws.  Their strategy is to use an approximation of the filtered
distributions $(\bbeta_t | \{y_s\}_{s=1}^t)$ when backwards sampling.  The CUBS
method is also $\mcO(T)$ as $T$ varies, but requires solving a non-linear
equation at each time step, which is time consuming.
...
\cite{ravines-etal-2006} find the CUBS approximation of \cite{west-etal-1985}
works well.  We did our own comparison and found that, indeed, the approach of
\cite{ravines-etal-2006} is superior to sampling blocks of disturbances.
However, none of these Metropolis-Hastings are completely satisfactory.  In
particular, \cite{ravines-etal-2006} is still time consuming, requiring $T$
non-linear solves for each MCMC iteration.
...
analog will coincide with a DLM so that one may make use of the FFBS.
...
using mixtures of Gaussians.  Their work has lead to data augmentation schemes
that satisfy (1) and (2), in an approximate though accurate sense, for binomial
and multinomial logistic regression \citep{fruhwirth-schnatter-fruhwirth-2007,
...
Fr\"{u}hwirth-Schnatter et al.'s cornucopia.)
