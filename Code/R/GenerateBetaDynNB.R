...
load("~/RPackage/BayesLogit/Code/R/DataSets/DynNB-synth-1.RData")

all.data = list(
  synth1 = list("y"=dyn.nb.1$y, "X"=dyn.nb.1$X, "with.iota"=dyn.nb.1$with.iota)
  )
J = length(all.data)
...
if (method=="PG") {
  ...
  source("~/RPackage/BayesLogit/Code/R/DynNBPG.R");
}
if (method=="FS") {
  if(!is.loaded("FSF_nmix.so")) dyn.load("~/RPackage/BayesLogit/Code/R/FSF_nmix.so")
  source("~/RPackage/BayesLogit/Code/R/DynNBFS-2009.R")
}
