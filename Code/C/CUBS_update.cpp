	   ...
	   int (*gsl_transform) (const gsl_vector*, void*, gsl_vector*))
{
  ...
  if (msg != GSL_SUCCESS) {
    fprintf(stderr, "CUBS_udpate.cpp::solver Error %i.  Break on %i.\n", msg, i);
    fprintf(stderr, "error: %s\n", gsl_strerror (msg));
    fprintf(stderr, "Init: r=%g, s=%g, f=%g, q=%g\n", ival[0], ival[1], fq[0], fq[1]);
    fprintf(stderr, "Exit: r=%g, s=%g, ", s->x->data[0], s->x->data[1]);
    fprintf(stderr, "F0=%g, F1=%g, ", s->f->data[0], s->f->data[1]);
...
void CUBSSolver::solve(const double* fq, double* rs, double epsabs, double epsrel, int max_iter)
{
  ...
  // You can turn off GSL error handling so it doesn't crash things.
  if (msg != GSL_SUCCESS) {
    fprintf(stderr, "CUBSSolver::solve: Error %i.  Break on %i.\n", msg, i);
    fprintf(stderr, "error: %s\n", gsl_strerror (msg));
    fprintf(stderr, "r=%g, s=%g\n", s->x->data[0], s->x->data[1]);
    fprintf(stderr, "f=%g, q=%g\n", s->f->data[0], s->f->data[1]);
  }
